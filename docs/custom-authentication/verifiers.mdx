...
It is possible to combine these attestation methods in AND/OR ways. For example you could combine a Google AND GitHub login to access a key or vice
versa can be done for Google OR GitHub. For these aggregate verifiers, its important to note that a user's `Verifier ID` must be known ahead of time,
as keys in Web3Auth are append only. Meaning to assign to a key of a user with the aggregate verifier, Google OR GitHub, we must know the user's gmail
account and discord account ahead of time.
...
9. Finally, click on `Create` to deploy the verifier. ![Login Providers list on Web3Auth Dashboard](/dashboard/verifiers/aggregate-verifier/av-5.png)
...
const openloginAdapter = new OpenloginAdapter({
  adapterSettings: {
    ...
    loginConfig: {
      ...
      jwt: {
        verifier: "YOUR_AGGREGATE_VERIFIER_NAME", // Pass the Verifier name here. eg. w3a-aggregate-verifier
        verifierSubIdentifier: "YOUR_SECONG_SUB_VERIFIER_NAME", // Pass the Sub-Verifier here. eg w3a-auth0
...
// // When user clicks Google button, use this to Login with GitHub via Auth0
const web3authProvider = await web3auth.connectTo("openlogin", {
  loginProvider: "jwt",
});
```
