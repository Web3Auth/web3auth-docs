...
:::info
  `web3auth.connect` will not return provider if you are using `redirect` uxMode with openlogin or customauth adapter.
  Pre configured openlogin adapter package uses popup uxMode which will always return the provider.
:::
...
:::info
You will get different information about user based on the login method used by user. For ex: if user authenticates using social logins then you will get name, email and profile image of user whereas if user is using some wallet like metamask to login then you will get only `ethereum` address of user.
:::
