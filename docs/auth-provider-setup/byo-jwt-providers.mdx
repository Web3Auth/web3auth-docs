...
3. Convert the publicKey.pem file to JWKS.
   ...
   - One of the tools is https://russelldavies.github.io/jwk-creator/
     ...
     - PEM encoded key: p`aste-the-publicKey-pem-file-content-here`
...
4. To complete the process, you need to save the output as a `.json` file, host it on your server, and make sure it's available to the public. This
   will give you the `JWKS Endpoint`, which is required when setting up a Custom JWT Verifier on the Web3Auth Dashboard.
