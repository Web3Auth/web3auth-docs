...
\*Auth0 column also includes Custom JWT, AWS Cognito, and Firebase providers when the JWT user
identifier is email.

:::info Important

- **Facebook** cannot be aggregated with any other login provider, including Auth0.
- **Twitch** and **Discord** can only be aggregated with themselves (self-aggregate).
- **SMS Passwordless** can only be aggregated with itself (self-aggregate).
