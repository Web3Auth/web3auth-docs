...
This resolves an email address to an Solana public Address. Returns an account if it already exists on torus network. Creates a new wallet for that
...
- `Promise<string>` : Returns a promise which resolves to the Solana address associated with the email.
