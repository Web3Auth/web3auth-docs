...
The Fiat On-Ramp Aggregator, a key component of Web3Auth's Wallet Services, streamlines the process
of converting fiat to cryptocurrency. With integration requiring just a single line of code, it
supports over 100 payment methods.

:::note

The Fiat on Ramp supports both Externally Owned Accounts (EOA) and Account Abstraction (AA) accounts
...
- **Lower fees:** Save up to 70% on transaction fees by leveraging lower cost local payment
  providers

- **Localized Payment Options:** Offer familiar and real-time payment methods to boost user
  engagement and monetization

- **Higher Transaction Success Rates:** Address high failure rates (30-70%) of traditional methods
  like Visa/Mastercard in emerging markets by using reliable local payment options.

- **Enhanced Security:** Provide a secure and cost-effective solution for both users and merchants,
  as local payment methods often require 2-factor authentication (2FA) and eliminate chargeback
  costs.
...
![Coverage](/images/wallet-services/topup/topup-coverage.png)
...
![Providers](/images/wallet-services/topup/topup-providers.png)
...
<CommonQuestions
  questions={[
    "What is Web3Auth's Fiat On-Ramp Aggregator?",
    "What types of accounts and networks are supported by the on-ramp?",
    "What are the benefits of using Web3Auth's on-ramp solution?",
    "Which countries and payment methods are supported?",
    "What blockchain networks are supported for fiat on-ramping?",
    "How does the on-ramp handle different payment methods?",
    "What are the transaction success rates compared to traditional methods?",
    "How does the on-ramp support global users?",
    "What security measures are in place for transactions?",
    "How does the on-ramp reduce transaction fees?",
  ]}
/>
