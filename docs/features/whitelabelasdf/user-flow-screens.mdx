...
Web3Auth's User Flow Screens can be customized to match a dApp's branding using the
`OpenLoginAdapter` settings, providing more options for a personalized user experience.

![Web3Auth - User Flow Screens](/images/whitelabel/user-flow-screens.gif)

## Arguments

You can pass the `whiteLabel` parameters to `OpenLoginAdapter` with `@web3auth/modal`,
