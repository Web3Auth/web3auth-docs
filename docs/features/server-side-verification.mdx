...
The External Wallet's `idToken` payload is different than the Social login's `idToken`. During Social logins, we get the user's pub key, and in
...
- **JWT Validation:** The associated public key, available on a hosted website provided by Web3Auth, allows you to verify the token's authenticity and
  ...
  altered.
- **Proof of Wallet Ownership:** The JWT includes a claim that certifies the user's ownership of a specific wallet's public address, enabling a
  reliable method of user authentication on the server side.
