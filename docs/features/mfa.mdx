...
are tied to different authentication methods. These keys are extracted from the user's social accounts, devices, recovery phrases etc. and are used to
...
Web3Auth returns a field called `dappShare`, which is a 24-word seed phrase that can be used to reconstruct the private key.
