...
interface WhiteLabelParams {
  ...
  /**
   * Contact URL(s) (multiple languages are supported)
   */
  contactLink: {
    [lang: string]: string;
  };
  /**
   * Privacy Policy URL(s) (multiple languages are supported)
   */
  privacyPolicy: {
    [lang: string]: string;
  };
  /**
   * Custom translation texts, see example above for example custom translations
   */
  customTranslations: {
    [lang: string]: any;
  }
