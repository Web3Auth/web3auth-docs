...
Session Management is a major update released recently to enhance security. While logging in, we manage the logins via Session IDs and not store the
private keys in the local storage. We use these session ids for creating a Web3AuthState which will be used to keep track of the user state. This way,
the user only needs to login sometimes the app is opened.

The dApp can directly check if the user is logged in or not by checking for this Web3AuthState with the following code snippet.
