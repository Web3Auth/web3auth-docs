...
import ThresholdKey from "@site/src/common/sdk/selfHost/_threshold-key.mdx";
...
[Create a Verifier](/custom-authentication/verifiers) from the **Custom Auth** section of the
...
[initialisation](./initialisation) section, you need to configure the modules beforehand to make it work with tKey. Once that is done, the instance of
the respective module is available within your tKey instance and can be used for further operations.
...
Once you have generated the private key, you can use it further to make blockchain calls. The key generated by tKey is of the type `secp256k1` which
is compatible with EVM based blockchains, like Ethereum, Polygon and many others that use the same curve. Although, you can convert this key into
other curves as well and utlise it, for example, we have a dedicated package for converting this module to `ed25519` curve for usage in Solana and
other blockchains that use this curve.

In addition to that, we have dedicated provider packages for EVM and Solana Blockchain libraries. You can checkout their respective documentation
here:
