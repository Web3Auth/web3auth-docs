---
title: XRPL Private Key Provider
...
For getting the Chain Config for XRPL Chain, you need to use the `getXRPLChainConfig` function from
...
In this function, you can pass over your custom config as well as just the network type for web3auth
to generate a standard config according to your requirement.
...
If you are using `chainNamespace: "other"` while initializing `Web3Auth` or `Web3AuthNoModal` with
the `OpenloginAdapter`, you need to add the `privateKeyProvider` to the OpenLogin instance.
...
messages. Below are a few sample use cases to assist you in getting started:
...
The purpose of this function is to sign and broadcast a transaction onto the blockchain. Upon
successful execution, it will return the signature of the broadcasted transaction.
