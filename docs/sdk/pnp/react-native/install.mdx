...
such as push notifications, build, and updates. Although, the React Native PnP SDK is not compatible with "Expo Go" app. It is compatible only with
...
```bash npm2yarn
npm install --save empty-module readable-stream react-native-quick-crypto
```
