...
```bash npm2yarn
npm install --save empty-module stream-browserify crypto-browserify
```
...
:::info
You will have to create `metro.config.js` for Expo Managed Workflow, as it is not present by default. Also note that the React Native PnP SDK
is not compatible with "Expo Go" app. It is compatible only with Custom Dev Client and EAS builds. Please run `npx expo prebuild` before running
