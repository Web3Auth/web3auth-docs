...
The React Native PnP SDK V4 is not compatible with "Expo Go" app. It is compatible only with
...
project directly with Swift/Kotlin native modules. Check out the troubleshooting section for fixing common issues.
...
```bash npm2yarn
npm install --save empty-module readable-stream crypto-browserify
```
...
### Fixing `Buffer` issues in Bare workflow
...
Then, import the `globals.js` to `index.js` of your project.

```js title="index.js"
import "./globals.js";
```

### Fixing logout issues in iOS apps (applicable for both Bare and Expo managed workflow)
...
Then, add the following code to `index.js` of your project.
