...
```bash npm2yarn
npm install --save empty-module readable-stream crypto-browserify react-native-get-random-values
```
...
```js title="metro.config.js for expo managed workflow"
...
```js title="metro.config.js for bare workflow"
