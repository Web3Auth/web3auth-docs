...
The Web3Auth React Native PnP SDK is not compatible with the "Expo Go" app. It is compatible only with Custom Dev Client and EAS builds. Please refer to
...
- In iOS, you don't need to add `redirectUrl` as an iOS Custom URL Scheme, however, you may add it to your `Info.plist`, but it is not required.
