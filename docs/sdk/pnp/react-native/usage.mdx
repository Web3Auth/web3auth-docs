...
- `privKey()` - After log in, gets the private key of the user (default is `secp256k1` key).
- `ed25519Key()` - After log in, gets the `ed25519` key.
...
Along with v4, the `login()` method does not return any object containing the `userInfo`. In order to get the `userInfo` one needs to call the method
via the web3auth instance.
