...
Trigger the login flow that navigates the user to a browser model that allows them to login with the selected login provider.
...
Along with v4, the `login()` method does not return any object containing the `userInfo`. To get the `userInfo` one needs to call the method via the
web3auth instance.
...
Along with v4, `privKey()` method is added to the web3auth instance. This method returns the `secp256k1` private key.
...
Along with v4, `ed25519Key()` method is added to get the `ed25519` private key.
