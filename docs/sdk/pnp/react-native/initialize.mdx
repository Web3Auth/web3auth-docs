...
:::info NOTE
Now with `react-native-sdk` v4 `SecureStore` or `EncryptedStorage` gets passed with the Web3Auth constructor, depending on the workflow you are using. This is due to
the addition of session management without storing the private key in the device.
:::
