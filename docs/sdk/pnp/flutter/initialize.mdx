...
SDK using `init` and `initialize` function.
...
Initialize the `Web3AuthFlutter` plugin and configure the whiteLabel and custom authentication as
per your use.
...
The `Web3AuthFlutter` takes an object called `Web3AuthOptions` as input. Below are the available
fields of the `Web3AuthOptions` class.
