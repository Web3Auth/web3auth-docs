...
final Web3AuthResponse response = await Web3AuthFlutter.login(
  // highlight-start
    LoginParams(loginProvider: Provider.email_passwordless,
      extraLoginOptions: ExtraLoginOptions(login_hint: "hello@web3auth.io", additionalParams: additionalParams)));
  // highlight-end
```
...
Future<void> initPlatformState() async {
  ...
  loginConfig['jwt'] = LoginConfigItem(
    ...
    client_id: "CLIENT_ID", // Auth0, Google, Facebook, Twitch, Discord or Web3Auth Client ID
  );
  // highlight-end
...
A void object is returned on successful logout otherwise an error is thrown
