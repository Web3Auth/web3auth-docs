...
modal. Each adapter exposes the provider on successful user login which can be used to invoke RPC calls on the wallet and connected blockchain. You can
configure and use any of the adapters provided by Web3Auth, by just configuring them while initializing Web3AuthNoModal.
