...
initialize the Web3Auth Instance as mentioned in the
[modal docs](/sdk/pnp/web/modal/initialize#example) and
[no-modal docs](/sdk/pnp/web/no-modal/initialize#example). You can configure this instance with all
the options available in Web3Auth Modal package and set it up as you wish.
...
Checkout the various
[examples](https://github.com/Web3Auth/web3auth-pnp-examples/tree/main/web-no-modal-sdk/wagmi/wagmi-no-modal-example)
made for the wagmi connector using various UI Kits for better integration and user experience

:::

Here are a few examples of a wagmi client using both the `Web3AuthConnector` and the default
`InjectedConnector` respectively.
