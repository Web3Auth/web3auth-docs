...
If you are using `chainNamespace: "solana"` while initializing `Web3Auth` or `Web3AuthNoModal`, it will provide you with a Solana provider as
...
### Getting a provider from any `ed25519` private key.

`@web3auth/solana-provider` package can be used to get a solana compatible provider from any `ed25519` private key. You can use this package with our
SDKs to get an solana provider from the logged-in user's private key.
...
Web3Auth by default returns an `secp256k1` private key. However, for Solana, you need an `ed25519` key. We have made some additional libraries to help
you convert this `secp256k1` key to `ed25519` key. Further you can utlise the same to get a Solana Provider.
...
/*
privateKey: any secp256k1 private key.
*/
