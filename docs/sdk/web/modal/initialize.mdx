...
with Web3Auth Plug and Play Modal SDK, have a look at our [Whitelabeling SDK Reference](/sdk/web/web3auth/whitelabel).
...
modal UI supports a set of default adapters depends of the [`authMode`](/sdk/web/web3auth/initialize#web3authoptions) being used.
...
Since we're using the `@web3auth/modal`, ie. the Plug and Play UI SDK, the `loginConfig` should be corresponding to the socials mentioned in the
