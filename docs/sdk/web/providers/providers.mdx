...
provider you the choice to export your private key, which can be used further to connect to the blockchain network. However, to make it easier for
...
using RPC calls. By default Web3Auth providers exposes chain specific providers based on the `chainConfig` and `chainNamespace` field passed in the
constructor.
...
Read more about it in [**EVM Provider**](./evm)
...
Read more about it in [**Solana Provider**](./solana)
...
Read more about it in [**Other Provider**](./other)
...
Example below demonstrate usage of `eip155` chainNamespace and retreiving ethereum private key of logged in user.
...
Example below demonstrate usage of `solana` chainNamespace and retreiving private key of logged in user.
