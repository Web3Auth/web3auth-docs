...
- To use the `JWT` login, you need to put the jwt token into the `additionalParams` field of the `extraLoginOptions` as _HashMap_.
  ...
  val loginCompletableFuture: CompletableFuture<Web3AuthResponse> =
    web3Auth.login(LoginParams(selectedLoginProvider,
      ...
      extraLoginOptions = ExtraLoginOptions(
        additionalParams = hashMapOf ("id_token" to "your_jwt_token")
      )))
      // highlight-end
  ```
