...
dApp Share is a feature that allows you to store your recovery share somewhere secure.

```kotlin
web3Auth.login(LoginParams(selectedLoginProvider, dappShare = "share"))
```
...
class MainActivity : AppCompatActivity() {
	...
	private fun signIn() {
		val selectedLoginProvider = Provider.GOOGLE   // Can be GOOGLE, FACEBOOK, TWITCH etc.
		val loginCompletableFuture: CompletableFuture<Web3AuthResponse> = web3Auth.login(LoginParams(selectedLoginProvider, dappShare = "share"))
		loginCompletableFuture.whenComplete { loginResponse, error ->
			if (error == null) {
				// render logged in UI
			} else {
