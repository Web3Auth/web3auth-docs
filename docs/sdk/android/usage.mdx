...
Web3Auth's Android SDK is a set of libraries and tools that allow you to easily integrate Web3 with Android applications. You can perform login,
logout, and sign transactions using the SDK. You will also be able to use `MultifactorAuthentication` to add additional security to your application.
Enable `dappShare` to secure your device share somewhere secure. Also, has the option of selecting the `curve` you want to use for signing
transactions.
...
class MainActivity : AppCompatActivity() {
	...
	private fun signIn() {
		val selectedLoginProvider = Provider.GOOGLE   // Can be GOOGLE, FACEBOOK, TWITCH etc.
		val loginCompletableFuture: CompletableFuture<Web3AuthResponse> = web3Auth.login(LoginParams(selectedLoginProvider, dappShare = "<24 words seed phrase>"))
		loginCompletableFuture.whenComplete { loginResponse, error ->
			if (error == null) {
				// render logged in UI
			} else {
