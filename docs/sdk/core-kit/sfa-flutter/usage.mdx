...
- `getAggregateKey(LoginParams)` - Returns the aggregate private key using the `verifier`,
  `verifierId` & `idToken`.

## `getKey()`

To obtain a user's private key using the Web3Auth SFA Flutter SDK, you can call the `getKey()`
function.
...
To obtain a user's Torus key using the Web3Auth SFA Flutter SDK, you can call the
`getAggregateKey()` function.
