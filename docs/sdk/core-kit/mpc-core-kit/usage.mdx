...
- `loginWithOAuth()` - Logs in the user in OAuth Intrinsic Flow.
- `loginWithJWT()` - Logs in the user with a JWT-based ID Token.
- `logout()` - Logs out the User.
- `handleRedirectResult()` - Handles the redirect result from the OAuth Intrinsic Flow.
...
Sets the TSS Wallet Index for the User's account. This is a new feature that supports key derivation, allowing users to derive multiple private keys
from a single account for improved asset organization.
