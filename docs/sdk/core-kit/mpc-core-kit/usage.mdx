...
- [`_UNSAFE_exportTssKey()`](#export-tss-key) - Returns the user's secp256k1 tss private key.
- [`_UNSAFE_exportTssEd25519Seed()`](#export-tss-ed25519-seed) - Returns the user's ed25519 tss
  seed.
...
Returns the user's secp256k1 tss private key.
...
### Export TSS ed25519 seed

#### `_UNSAFE_exportTssEd25519Seed(): Promise<Buffer>`

Returns the user's ed25519 seed. Please note, the seed is only return when user imports the seed
while creating a new account. If TSS seed is not imported, the function will throw an error.

#### Usage

```tsx
const seed = await coreKitInstance._UNSAFE_exportTssEd25519Seed();
```
