...
| ---------------------- | --------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `verifier`             | `string`              | Yes       | Name of the verifier created on Web3Auth Dashboard. In the case of Aggregate Verifier, the name of the top-level aggregate verifier.       |
| `verifierId`           | `string`              | Yes       | Unique Identifier for the User. The verifier identifier field is set for the verifier/sub verifier. E.g. "sub" field in your JWT ID Token. |
| `idToken`              | `string`              | Yes       | The idToken received from the Auth Provider.                                                                                               |
| `subVerifier?`         | `string`              | No        | Name of the sub verifier in case of aggregate verifier setup. This field is mandatory in the case of an aggregate verifier.                |
