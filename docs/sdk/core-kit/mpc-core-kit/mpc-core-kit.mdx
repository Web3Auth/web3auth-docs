...
As of now, all the features of MPC are not available in the SDK. We are working on adding more features to the SDK and will be releasing them in the
consecutive updates.
...
The SDK can be used for production applications, however, we recommend getting regular updates from our team to ensure that you are using the latest
version of the SDK, in the correct flow.
