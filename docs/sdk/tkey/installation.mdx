...
import ServiceWorkerCode from "@site/src/common/sdk/selfHost/_sw-js.mdx";

---

While installing Web3Auth Self Host, we recommend installing the `tkey/default` SDK. This package contains all the needed functionalities of the
`tkey` SDK.
...
network-related features. Web3Auth Self Host SDK needs a service worker relative to `baseUrl` to capture the auth redirect at `redirectPathName` path.
...
- Using service worker is optional, but highly recommended. You can skip it by passing `skipSw` param while [initialising tKey](./initialisation).

- Service worker is needed if you are using `redirect` uxMode within your
  [Service Provider configuration](./initialisation#configuring-service-provider).
...
[self-host-react-example](https://github.com/Web3Auth/examples/blob/main/self-host/self-host-react-example/public/serviceworker/sw.js).
