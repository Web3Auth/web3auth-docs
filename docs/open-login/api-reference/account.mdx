...
- `params` - `LoginParams` \(optional\) : The login options. Used to specify a
  type of login.
  - `loginProvider` - `string` : The loginProvider name. Supported loginProviders are :
    `coming soon...`
...
- `Promise<string[]>` : Returns a promise which resolves to the private key of the user.
