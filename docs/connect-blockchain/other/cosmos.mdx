...
user's `account`, `chain ID`, fetch `balance`, `send transaction`, etc. We have highlighted a few methods here to get you started quickly on that.

:::note

This reference is for the `Web`, however, you can use Cosmos on other Mobile and Gaming Platforms as well. Please follow our reference for
[EVM Blockchains](/connect-blockchain/evm/), and similarly use Cosmos libraries that support the platforms to use the private key and make blockchain
calls accordingly.
...
const chainConfig = {
  ...
  blockExplorer: "https://www.mintscan.io/cosmos", // Please add in your custom URL
  ticker: "ATOM",
  tickerName: "Cosmos",
};
```
