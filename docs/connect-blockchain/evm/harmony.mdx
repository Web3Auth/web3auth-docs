...
[Harmony](https://www.harmony.one/) blockchain calls like getting the user's `account`, fetching `balance`, `sign transaction`, `send transaction`,
`read` from and `write` to the smart contract, etc. We have highlighted a few here to get you started quickly on that.

:::info

This reference is for the `Web`, however, you can use Harmony on `Android`, `iOS`, `React Native`, `Flutter`, & `Unity` as well. Please follow our
reference for [Ethereum](/connect-blockchain/ethereum/), and change the RPC Provider URL & configurations corresponding to Harmony.
