...
The `rpcUrl` used will determine the chain-specific details. We have documented for `ETH Mainnet`, to configure it with other EVM Chains, simply
change the rpcUrl to its corresponding chain.
...
```tsx
private val rpcUrl = 'https://rpc.ankr.com/eth_sepolia'; // Sepolia Testnet
```
