...
& `websocketUrl` for Devnet-beta. To interact with Testnet or Mainnet, you can simply change the `rpcUrl` and `websocketUrl`.

### Initializing Solana SDK

In the below code block, we'll initialize the `SolanaClient` using the Devnet-beta rpc and websocket urls. We'll also
