...
While using the Web3Auth Flutter SDK, you can retrive the Ed25519 private key upon successful authentication. This private key can be used
to derive user's public address and interact with [Solana](https://ethereum.org/) chain. We have highlighted a few methods here for getting
you started quickly on that.

:::note

This reference is for the `Flutter`, however, you can use Solana on `Android`, `iOS`, `React Native`, `Web`, & `Unity` as well. Please follow our
reference for [Ethereum](/connect-blockchain/ethereum/), and similarly use Solana libraries that support the respective platforms to interact with
Solana blockchain.
...
If who want to skip straight to the code, you can find it on GitHub in [Web3Auth PnP examples](https://github.com/Web3Auth/web3auth-pnp-examples/tree/main/flutter).
...
we're using [solana](https://pub.dev/packages/solana) to demonstrate how to interact with Solana chain using Web3Auth.

```dart
flutter pub add solana get_it hex
```

> Note: We will also be using `get_it` package to for dependency injection, and `hex` package to perform
> Hex encoding and decoding.

## Initialize

To Initialize the `SolanaClient` we require `rpcUrl` and `websocketUrl`. The `rpcUrl` will provide a gateway & protocol
to interact with Solana cluster while sending requests and receving response. For this example, we are using `rpcUrl`
& `websocketUrl` for Mainnet-beta. To interact with Testnet or Devnet, you can simply change the `rpcUrl` and `websocketUrl`.

### Initializing Solana SDK

In the below code block, we'll initialize the `SolanaClient` using the Mainnet-beta rpc and websocket urls. We'll also
register the instance in GetIt for it to be accessed anywhere using service locator.
...
After logging in, the Web3Auth instance will provide you with information regarding the user that
is logged in. This information is obtained directly from the JWT token and is not stored by Web3Auth.
Therefore, this information can only be accessed through social logins after the user has logged into your application.
...
We can use `getEd25519PrivKey` method in Web3Auth to retrive the priavte key for the Solana ecosystem. In the below
code block, we'll use the Ed25519 private key to retive user's public address, and solana balance. We'll use `SolanaProvider`
class to interact with Solana cluster and fetch user balance.
...
Let's now go through how can we sign the transaction. In the below code block, we'll add a new method inside
`SolanaProvider` we setup earlier to help us sign a transfer transaction. After successful implementation,
we can use the method in `HomeScreen`.
...
Once created, we can use the method in `HomeScreen` to transfer SOL. Upon successful transfer,
we'll also refresh the balance of the user.
...
`getEd25519PrivKey` is used to fetch the Ed25519 private key of the logged in user.
