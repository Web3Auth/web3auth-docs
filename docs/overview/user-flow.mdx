...
Web3Auth enables simple Web2 flows for Web3 onboarding. However, as good as that sounds, this process is has a few moving parts for the developers to
...
of Threshold Cryptography (with Shamir Secret Sharing), users only need a minimun of 2 out of nth number of keyshares to get back their private key.
This allow users to have control over their key and at the same time prevent account loss from a single point of failure (ie, they can lose their
device and still have access to their account). Further to create a social login share, a user has to use the Web3Auth Auth Network, where the key
generation happens through a 5/9 consensus system. This ensures that the keys are always non custodial to Web3Auth, Social Login Providers or any
other party holding a key share.
...
- **Types of Shares**
  ...
  - **Trusted Device Share** - Share attached with the user's device. This share is stored wither in the browser local storage, or as a dApp Share
    within the particular application local context.
