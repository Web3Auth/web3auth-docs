...
Web3Auth v9 to v10. This is a supplementary guide to the main
[v9 to v10 migration guide](./modal-v9-to-v10.mdx).
...
- **Action:** Rebuild your modal display logic using the new `modalConfig` structure in
  ...
  Deprecated v9 `WALLET_ADAPTERS` enum is replaced by `WALLET_CONNECTORS` and specific login method
  keys.

### 3. Auth Adapter Whitelabeling in v9: No longer supported

- In v9, `whiteLabel` settings in an `@web3auth/auth-adapter` instance could customize intermediate
  auth screens (e.g., social login pop-ups).
...
- **Action:** Remove v9 `AuthAdapter` `whiteLabel` configurations. Ensure your dashboard branding is
  comprehensive.
