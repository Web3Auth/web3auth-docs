...
- With v5, when sending the whitelabel object while initialization, please keep in mind that the
  `name` parameter signifying the name of the app has been changed to `appName`.
- The `dark` parameter that used to accept a boolean value to switch between dark or light mode has
  been changed to `mode` that accepts a string value of either `light` or `dark` or `auto`.
- The `theme` parameter now accepts an object with key-value pairs, where the value corresponds to
  the color for a specific set of keys.
