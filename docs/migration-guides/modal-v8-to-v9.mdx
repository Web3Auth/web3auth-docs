...
This migration guide provides steps for upgrading from version v8 to v9 of the Web3Auth PnP Modal
...
injected wallets available in the browser. Starting from v9, it will now show a list of
WalletConnect-compatible wallets, enabling users to connect via WalletConnect if a wallet is not
already installed. Please look at the image below.

![getDefaultExternalAdapters image](/images/migration-guides/default-adapter.png)

Additionally, a new `getInjectedAdapters` function has been introduced, which detects injected
wallets and, if none are available, prompts users with an option to install one. Refer to the image
below for details: ![getInjectedAdapters image](/images/migration-guides/injected-adapter.png)

The key difference between `getInjectedAdapters` and `getDefaultExternalAdapters` lies in their
handling of missing wallets. While `getInjectedAdapters` provides users with an installation option
when no wallet is found, `getDefaultExternalAdapters` allows users to connect via WalletConnect in
the absence of an installed wallet.
...
Starting with v9, the `initModal` method for initializing the Modal SDK has been removed.
Initialization is now handled internally by the hooks.
