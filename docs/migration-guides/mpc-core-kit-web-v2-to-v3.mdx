...
In v3, we try to improve the developer experience by removing following parameters:

- `setupProviderOnInit` was used to internally setup provider after successful login. In latest
  version provider is not setup internally, and developers will need to explicitly setup provider.
  Please check [EthereumSigningProvider Set up](#ethereumsigningprovider-set-up)
- `chainConfig` was used internally use to setup provider after successful login. Since,
  `setupProviderOnInit` is removed, `chainConfig` became obsolete.
...
- `asyncStorageKey` was used to provide custom storage for the SDK. This is now removed in support
  for `storage` parameter to improve developer experience.

Along with removing the above parameters, following mandatory parameters are now added in v3.

- `storage`: Developers can now use storage for both, local and async storage.
  ...
  - If you used to pass `memory` for the `SupportedStorageType`, you need to pass a new instance of
    `MemoryStorage`.
  - If you used `asyncStorageKey`, you can need to pass `IAsyncStorage` instance.
...
- `tssLib` is now a mandatory parameter to define the tss signing library to use. Based on the
  ...
  - For generating ECDSA signature, you should use
    [@toruslabs/tss-dkls-lib](https://www.npmjs.com/package/@toruslabs/tss-dkls-lib)
  - For generating EdDSA signature, you should use
