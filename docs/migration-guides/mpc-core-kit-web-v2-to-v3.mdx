...
Web SDK. The guide outlines significant changes and enhancements, including the support of ed25519.
...
`storageKey`, and `asyncStorageKey` parameters. Developers can now use the mandatory `storage`
...
In v3, the `getWebBrowserFactor` is removed. Developers can now use `getDeviceFactor` to retrieve
user's device factor if available.
