...
This migration guide provides steps for upgrading from version v1 to v2 of the Web3Auth No Modal SDK. The guide outlines breaking changes in the
return type of the methods of `SingleFactAuthFlutter` class.
...
In v2, the `initialize` method will return the `Future<TorusKey?>` instead of `Future<String>`. The method will return `null` if the user doesn't have
any active session present.
