...
For all Non-EVM & Non-Solana chains use the `CommonPrivateKeyProvider` from the `@web3auth/base-provider`package. Unlike, `EthereumPrivateKeyProvider`
and `SolanaPrivateKeyProvider`, `CommonPrivateKeyProvider` does not require any parameter at the time of initialization.
