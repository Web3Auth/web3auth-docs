...
`ChainConfig` now requires a `logo` parameter for the chain's logo and the parameter formerly called `blockExplorer` has been renamed to
`blockExplorerUrl`. Additionally, `isTestnet` has been introduced which can be used to define whether the network is testnet or not.
