...
- **Which logins should my application support?**
  ...
  - You can select from [this list](/custom-authentication/verifiers#supported-logins) or use your own custom authentication. There are some nuances
...
- **Should different logins lead to the same key?**
  ...
  - You can choose to connect logins with the same `Verifier ID`. For example if a user uses generic email logins and a gmail login to login on
    ...
    Read more about aggregating logins [here](/custom-authentication/verifiers#aggregating-loginsverifiers).
