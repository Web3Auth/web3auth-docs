...
After a user is logged in using one of the supported [Wallets](/sdk/pnp/web/adapters/#currently-available-wallet-adapters). One can obtain the
`idToken` by calling `web3auth.authenticateUser()` of Web3Auth.
