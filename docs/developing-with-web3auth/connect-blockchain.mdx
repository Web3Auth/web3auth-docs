...
- `eip155`:- `eip155` chainNamespace will return a `EIP1193` compatible provider once user is logged in.
- `solana`:- `solana` chainNamespace will return a provider with solana chain connection as described [here](/api-reference/web/providers/solana)
- `other`:- If you want to use any other chain except Solana or EVM chains, for ex: Starknet. You can specify the value of `chainNamespace` field as
  `other` in the `Web3Auth` sdk constructor,
...
Also other chain config fields except `chainNamespace` are not required while using `other` chainNamespace.
