...
Your jwt token [header](https://github.com/auth0/node-jsonwebtoken#jwtsignpayload-secretorprivatekey-options-callback) must contain the `kid` field and the payload data must contain `iat` field.
