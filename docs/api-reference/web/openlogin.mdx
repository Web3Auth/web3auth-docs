...
import OpenloginOptions from "./common/_openlogin-options.mdx";
import OpenloginLoginParams from "./common/_openlogin-login-params.mdx";
...
import ConnectTo from "./common/_connect-to.mdx";
import ExtraLoginOptions from "./common/_extra-login-options.mdx";
...
import ConnectToExample from "./common/_connect-to-example.mdx";
...
Read more about how to pass `loginParams` to `connectTo` in its respective section: [`web3auth/core`](./core/usage#logging-in-the-user)
...
When we're using the `@web3auth/web3auth`, ie. the Plug and Play UI SDK, the `loginConfig` should be corresponding to the socials mentioned in the
...
Know more about the `connectTo` function in the [`Usage API Reference`](./usage#logging-in-the-user)
