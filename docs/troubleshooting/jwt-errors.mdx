...
the validity of the **id_token** and compare its payload value to the JWKS provided by either the Auth provider or your custom JWKS. So even with a
refresh token or new id_token, it would be another login call.
...
Although we do not support Refresh Tokens, we do offer session management. The **Session Management** feature allows checking and maintaining existing
sessions with Web3Auth.
