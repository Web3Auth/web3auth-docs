...
When integrating Web3Auth with Auth0 the flow looks something like this:

<img style={{ display: "block", maxHeight: "600px", margin: "20px auto" }} src="/contents/auth0-flow.png" />
...
- A basic knowledge of JavaScript is required to use Web3Auth SDK for Web apps.

- For the Web3Auth Mobile SDKs, you have a choice between iOS, Android, React Native & Flutter. Please refer to the
  [Web3Auth API Reference](https://docs.web3auth.io/api-reference) for more information.
...
- Create an Auth0 tenant and configure a Single Page Web Application (SPA for Web) or Native (for Mobile) from
  ...
  - Please note that Regular Web Applications are also supported, however for the guide here, we’re preferring SPA since its implicit flow doesn’t
    require an application to host a server. You can navigate to the [additional section](#code-flow-with-rwa) below which talks about the code flow
    with RWA.

- Create an account on the [Web3Auth Dashboard](https://dashboard.web3auth.io)
...
- Add the following redirect URLs for your app to the Allowed Web Origins field when configuring your Auth0 Application.
  - Additional to your own redirect URLs for the application, please add
...
### Setting up your Web3Auth Dashboard

- Create a Project from the **Plug and Play** Section of the [Web3Auth Developer Dashboard](https://dashboard.web3auth.io/).

  <img style={{ display: "block", maxHeight: "600px", margin: "20px auto" }} src="/contents/auth0-guide-01.png" />

  - Enter your desired Project name
  - Select the blockchain(s) you'll be building this project on. For interoperability with Torus Wallet, you've an option of allowing user's private
    key to be used in other application using Torus Wallet. We currently have this option across `EVM` and `Solana` and `Casper` blockchains.

    <img float="left" style={{ display: "block", maxHeight: "400px", margin: "20px auto" }} src="/contents/auth0-guide-02.png" />

  - Finally, once you create the project, you've the option to whitelist your URLs for the project. Please whitelist the domains where you project
    will be hosted.
    <img style={{ display: "block", maxHeight: "400px", margin: "20px auto" }} src="/contents/auth0-guide-03.png" />
...
- Create a Verifier from the **Custom Auth** Section of the [Web3Auth Developer Dashboard](https://dashboard.web3auth.io/) with following
  ...
  - Paste the `clientId` and `domain` from the Auth0 SPA you create in the above steps. This will be used to validate the JWT token issued by Auth0.
    <img style={{ display: "block", maxHeight: "500px", margin: "20px auto" }} src="/contents/auth0-guide-04.png" />
...
For this guide here, we will be talking through the [Web3Auth Plug and Play SDK](/api-reference/web/plugnplay) and using the
[OpenLogin Provider](/api-reference/web/adapters/openlogin) alongside it to enable Custom Authentication through Auth0.

### Setting up your base React project for using Web3 libraries:

To setup this project locally, create a new react project using the following command:
...
```shell
npx create-react-app auth0-app --template typescript
```
...
```shell
yarn create react-app auth0-app --template typescript
```
...
Once your project is created, follow the following steps to setup the repository properly

- You'll be facing some error with Webpack 5, once we install the dependencies for interacting with the blockchain. This is because certain packages
  are not available in the browser environment, and we need to polyfill them manually.

  - To fix this, first install react-app-rewired and the missing modules
...
- Add a `config-overrides.js` file to the root folder of your project and copy the following code to the `config-overrides.js` file:
...
- Finally, within your `package.json` change the scripts field for start, build and test. Instead of `react-scripts` replace it with
  `react-app-rewired`. It should look something like this:
...
Now, you're all set and ready for installing the dependencies.
...
This is the main **Plug and Play** package that contains the Web3Auth SDK with a login UI, giving you a simple way of implementing Web3Auth within
your interface.
...
- For Mobile, depending on the platform, there will be different steps to initialize the SDK. Refer to the respective Mobile SDKs,
  - [Android](https://docs.web3auth.io/api-reference/android/setting-up)
  - [iOS](https://docs.web3auth.io/api-reference/ios/setting-up)
  - [Flutter](https://docs.web3auth.io/api-reference/flutter/setting-up)
  - [React Native](https://docs.web3auth.io/api-reference/react-native/setting-up).
...
there directly from the Web3Auth SDK.
...
`web3auth.provider` to interact with your blockchain. You can use the [Provider SDKs](https://docs.web3auth.io/api-reference/web/providers/) to
perform RPC Calls to your blockchain.
...
- [Ethereum Provider](https://docs.web3auth.io/api-reference/web/providers/evm) gives you the capability of making RPC calls to the EVM compatible
  blockchains.
- [Solana Provider](https://docs.web3auth.io/api-reference/web/providers/solana) gives you the capability of making RPC calls to the Solana
  blockchain.
- If you want to use any other chain except Solana or EVM chains, for ex: Starknet, you can specify the value of `chainNamespace` field as other in
  the Web3Auth SDK Constructor. Refer to: [Using other blockchains](https://docs.web3auth.io/api-reference/web/providers/#other)
...
[quick-starts](https://docs.web3auth.io/quick-start?lang=react&chain=eth&customAuthentication=yes&customLogin=no&whitelabel=no) for your respective
framework.
