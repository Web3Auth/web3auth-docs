---
title: Using Firebase with Web3Auth SFA Web
...
<SEO
  ...
  slug="/content-hub/guides/sfa-rn-bare-firebase"
/>
...
- The first step for the application is to get the user authenticated by a login provider(here, Firebase) and get the user's `idToken`.
...
- If the user has not enabled MFA, we use the `@web3auth/single-factor-auth-react-native` SDK to retrieve the private key shares from the Web3Auth
  auth network nodes and reconstruct the private key.
...
##### Instantiate the Web3Auth SFA Web SDK
