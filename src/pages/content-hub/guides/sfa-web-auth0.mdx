...
import SFAWebInstantiation from "@site/src/common/guides/_instantiation-w3a-sfa-web.mdx";
...
import SfaWebIntialization from "@site/src/common/guides/_sfa-web-initialization.mdx";
...
- The first step for the application is to get the user authenticated by a login provider(here, Auth0) and get the user's `idToken`.
...
To use the Auth0 SDK, we need to wrap our app with the `Auth0Provider` component. This component provides the Auth0 `domain`, `clientId` and
`authorizationParams` to the app.
