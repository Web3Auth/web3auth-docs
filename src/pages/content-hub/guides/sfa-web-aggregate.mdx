---
title: Aggregate Auth0 & Google with Web3Auth SFA Web
...
<SEO
  ...
  slug="/content-hub/guides/sfa-web-aggregate"
/>

This guide will cover the basics of how to set up your Web3Auth SDK, Auth0 and Google account for the integration and provide you with the links on
...
- The first step for the application is to get the user authenticated by a login provider(here, Auth0 & Google) and get the user's `idToken`.
...
To use the Auth0 and Google SDK, we need to wrap our app with the `GoogleOAuthProvider` and `Auth0Provider` components. This component provides the
Google `clientId` and the Auth0 `domain`, `clientId` and `authorizationParams` to the app.
...
Alongside the `Web3Auth` and `OpenloginAdapter` you need the above-mentioned packages from `@web3auth/base` for different initializations mentioned
