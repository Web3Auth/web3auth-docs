...
type: guide
tags: [web, "@web3auth/web3auth", mpc, google, whitelabel, react, ethereum]
date: 10th Aug 2022
...
Instead, these partial keys stored all across the different locations and your device are used to make partial signatures for your message/
...
For this guide, we'll be using a React application to demostrate how the Web3Auth MPC α (Alpha) SDK works. You can use any other framework of your
choice using any of the other Web3Auth Guides/ Examples are accordingly just modify the functions to make it work.
...
1. Multi Factor Authentication needs to be mandatory, this is done to ensure that the user has a backup of their key in case they lose their device.
   Please note that the email flow might be buggy right now, you can directly copy the backup phrase and save it somewhere for testing purposes.
