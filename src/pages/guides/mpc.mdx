...
reconstructed in the frontend using threshold signatures, with the new Web3Auth MPC (Multi Party Computation) architecture, it is **never
reconstructed**. Instead, these partial keys are stored all across the different locations, and your device is used to make partial signatures for
your message/ transaction. These are finally returned to the frontend where using TSS (Threshold Signature Scheme), these signatures are combined to
make a final signature. You can use this finally signed message/transaction to make a transaction on the blockchain.
...
As you can notice in this diagram above, the final output, ie. the User's TSS Account is generated in multiple stages within the whole infrastructure.
Since, this is an TSS- MPC based infrastructure, you don't get back a private key, but signatures that can be used to make transactions on the
blockchain. Let's understand each of the these stages in detail.
...
This is the primary way for a user to access their account. This step involves the authentication using a user's preferred social login provider. The
idToken received the social login provider here is passed over to the Web3Auth Auth Network to generate the TSS Shares in the Nodes. By default, these
nodes have a threshold of 3/5 which can be customised according to requirements. When a user logs in, the Auth Network generates signatures
corresponding to the TSS Shares in the nodes and returns them back to the user's end. These signatures are then used alongside other shares to
generate the final TSS Account signatures.

##### Device Factor

This is the second factor that is used to access the user's account. This step involves generation of a TSS Share on the user's device and using that
to generate a final signature for the TSS Account alongside the social login factor. This makes sure the user is logging in using their trusted device
and a proper non custodial setup is maintained.

##### Backup Factor

A user has a choice to generate as many backup factors as needed to access their account. This step involves generation of a TSS Share on the user's
end and storing them in which ever location they prefer. This share can be used similarly to the device share to generate a final signature for the
TSS Account alongside the social login factor and/or device factor.
...
the Auth Network, and 2/3 for the user's device front. This is to ensure high availability and ease of access on both ends alongside optimum security.
Both these thresholds can be customised according to the requirements.
...
reconstructed** and always stay decentralised and secure.
...
for its encryption/decryption capabilities. It utilizes basic Shamir’s Secret Sharing and is initially generated on a users’s front-end. The metadata
key is present to make sure that the infrastructure is backwards compatible with the existing Web3Auth architecture which utilizes SSS (Shamir’s
Secret Sharing) as the main key reconstruction scheme.
...
unique to each factor generated user's device and backups like users' phone, chrome extension, on their cloud, assisting third parties etc.

As shares to the TSS Key and/or Metadata Key may rotate, Factor Keys allow for a consistant secret to be saved in these different locations.

:::note

For this guide here, we will be talking through the **Web3Auth tKey JS MPC (Beta) SDK** and showing you a basic example of how to use it.
...
export const tKey = new ThresholdKey({
  enableLogging: true, // optional, used to logging all info, default is false
  serviceProvider: torusSp as any,
  storageLayer: storageLayer as any,
  manualSync: true, // needed for atomisation of tKey
  modules: {
...
[Github Example](https://raw.githubusercontent.com/Web3Auth/web3auth-core-kit-examples/main/tkey/tkey-mpc-beta-react-popup-example/src/utils.ts) or
the code below.
...
Since we are use the tKey Core MPC Beta SDK, we need to implement the device storage functions ourselves. We'll be using the local storage to store
the device share for this example.
...
const initializeNewKey = async () => {
  ...
  try {
    ...
    loginResponse = await triggerLogin();
    setOAuthShare(loginResponse.privateKey); // This private key is the OAuth Metadata Share
