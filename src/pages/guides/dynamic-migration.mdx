---
title: Migrate from Dynamic to Web3Auth
image: "guides/banners/dynamic-migration.png"
description: Migration guide from Dynamic to Web3Auth.
type: guide
...
<SEO
  title="Migrate from Dynamic to Web3Auth"
  description="Migration guide from Dynamic to Web3Auth."
  image="https://web3auth.io/docs/guides/banners/dynamic-migration.png"
  slug="/guides/dynmaic-migration"
/>

In this guide, we'll compare Dynamic and Web3Auth and explain how to migrate your existing users
from Dynamic to Web3Auth.
...
Both Web3Auth and Dynamic have made significant strides in addressing the Web3 onboarding challenge.
...
| ---------------------- | ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Wallet Management      | Dynamic only supports the SSS, and doesn't have MPC TSS support.                                        | Web3Auth supports both SSS and MPC TSS to manage user's private key.                                                                                                                                                |
| Multi-chain support    | Dynamic as of now only supports EVM compatible networks and Solana.                                     | Web3Auth is chain agnostic, it supports both secp256k1 and ed25519 curves.                                                                                                                                          |
| Multi-platform support | Dynamic only supports Web, and React Native.                                                            | Web3Auth supports multiple platforms, including Web, React Native, Unity, Unreal, Flutter, Swift, and Android. [Checkout documentation for more details](http://localhost:3000/docs/sdk?product=Plug%20and%20Play). |
...
| Whitelabel             | Dynamic has end to end white-labelling options.                                                         | Web3Auth also has end to end white-labelling options.                                                                                                                                                               |

## How to migrate to Web3Auth

Now that we've discussed the advantages of Web3Auth, let's explore the steps to migrate from Dynamic
to Web3Auth. We'll cover most scenarios, but if we miss any, please don't hesitate to reach out to
us in comments.

### External Wallets

If you are using Dynamic with branded wallets and are looking to integrate with Web3Auth, the
migration process is quite straightforward. The branded wallets is the terminology used by Dynamic
for external wallets. Web3Auth offers external multiple adapters to connect with external wallets,
along with social login options for added convenience. Along with external wallets, Web3Auth also
supports application wallets like Torus wallet.
...
If you are using a Dynamic embedded wallet as a signer for an ERC 4337 (Account Abstraction) wallet,
...
remove the Dynamic signer if preferred.
...
Considering you have already created the `ECDSAProvider` instance using Dynamic signer, you can simply use the `changeOwner` method to change the
Smart Account owner.
...
Considering you have already created the smart account instance using Dynamic signer, you can simply use the `transferOwnership` method to change the
Smart Account owner.
...
Considering you have already created the `LightAccount` instance using Dynamic signer, you can simply use the `transferOwnership` method to change the
Smart Account owner.
...
If you are using Dynmaic embedded wallet, there are couple of options which can be preferred for the
migration.
...
To facilitate the fund transfer using the migration tool, you can utilize our Wallet Pregeneration
feature to determine the user's wallet address without integrating our SDKs. By leveraging wallet
pregeneration, you can handle the migration in the backend seamlessly. Afterward, you can use the
PnP/ SFA SDK to log users in through Web3Auth.
...
with a Dynamic embedded wallet, you can seamlessly transition them to Web3Auth while retaining the
same account.

The first step is to export the Dynamic embedded wallet private key. To do this, you can use the
`initExportProcess` method from the `useEmbeddedReveal` hook. This will open the modal for users to
copy their private key.
[For more details on exporting accounts, learn more here](https://docs.dynamic.xyz/wallets/embedded-wallets/export).

Please note that there is no internal method to export the private key in Dynamic. Only the user can
access their full private key. When you use the `initExportProcess` method, it will display the
