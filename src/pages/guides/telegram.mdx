...
[Install the PnP packages](https://web3auth.io/docs/sdk/pnp/web/no-modal/install) and then import it
and configure the Web3Auth PnP No Modal SDK with the Telegram verifier.
...
[Install the SFA Node packages](https://web3auth.io/docs/sdk/core-kit/sfa-node/install) and then
import it and configure the Web3Auth SFA Node SDK with the Telegram verifier.
...
`/callback`, update the function to use `getPrivateKey()` to generate the Ethereum private key. The
`getPrivateKey()` is a helper function that uses the JWT token and Telegram user ID to generate the
Ethereum private key. For simplicity, we are returning the Ethereum private key and public address
to callback but you can modify it as per your requirement.
...
[Install the SFA Web packages](https://web3auth.io/docs/sdk/core-kit/sfa-web/install) and then
import it and configure the Web3Auth SFA Web SDK with the Telegram verifier in your React or any
