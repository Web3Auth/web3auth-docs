...
for authentication. We will utilize the Telegram Login Widget for user authentication and then
employ Web3Auth to create Ethereum private keys and public addresses. In this guide, you will learn
how to create a Telegram bot, generate a JWKS file, and establish a custom JWT verifier on Web3Auth.
Finally, we will demonstrate how to implement the Telegram OAuth flow for user authentication and
generate Ethereum/Solana keys using
[Web3Auth Plug and Play](https://web3auth.io/docs/sdk/pnp/web/no-modal/install) and
[Single Factor Auth](https://web3auth.io/docs/sdk/core-kit/sfa-web/install) SDKs.
...
2. [Create a JWKS file for your public key used for JWT signing](https://web3auth.io/docs/auth-provider-setup/byo-jwt-provider#how-to-convert-pem-to-jwks)
   and host it on a public endpoint.
3. [Set up a custom JWT verifier](https://web3auth.io/docs/auth-provider-setup/byo-jwt-provider#set-up-custom-jwt-verifier)
   for Telegram on the Web3Auth Dashboard.
...
6. Pass the JWT token to Web3Auth for generating Ethereum/Solana keys using the Web3Auth Plug and
   Play and Single Factor Auth SDKs.
...
- Telegram Login example with Web3Auth PnP SDK
  ...
  - [Live Demo](https://w3a-telegram-login.vercel.app)
...
2. [Custom JWT verifier for Telegram](https://web3auth.io/docs/auth-provider-setup/byo-jwt-provider#set-up-custom-jwt-verifier)
...
[here](https://github.com/Web3Auth/web3auth-pnp-examples/tree/main/web-no-modal-sdk/custom-authentication/single-verifier-examples/telegram-no-modal-example)
and the live demo [here](https://w3a-telegram-server.vercel.app/login).

## Edit Telegram Bot to set the domain

To ensure that the Telegram Login Widget works correctly, you'll need to set the domain of your
server.

[Check this guide on editing the Telegram Bot](https://core.telegram.org/bots/features#edit-bots) to
set the domain.

Simply select your Bot from the BotFather and click on the `Bot Settings` option. Then, click on the
`Domain` option, then click on `Set Domain` and enter the domain of your server. You'll see a
message saying `Success! Domain updated.`.
...
Import the required packages and configure the Web3Auth SDK with the Telegram verifier.
...
The `/login` route will initiate the Telegram OAuth flow. The code snippet remains the same. For
`/callback`, update the function to use `getPrivateKey()` to generate the Ethereum private key.
...
the login by calling the `connect` method with the verifier details along with the JWT token.

#### Set up the Web3Auth SFA Web SDK

Import the required packages and configure the Web3Auth SDK with the Telegram verifier in your React
or any other frontend project.
...
The above code snippets demonstrate how to use the Web3Auth SFA Web SDK with the Telegram OAuth
flow. You can find the complete example code in
[this GitHub repository](https://github.com/Web3Auth/web3auth-pnp-examples/tree/main/web-no-modal-sdk/custom-authentication/single-verifier-examples/telegram-no-modal-example).
