---
title: Using Web3Auth without redirect
image: "/docs/contents/guides/banners/one-key-flow.png"
description: Learn how to use Web3Auth to authenticate users without redirecting.
type: guide
...
Adding Web3Auth Plug and Play SDKs to your application is a great way to authenticate users and generating their private key. The users are redirected
to OpenLogin where they can sign in with their favourite social login or a custom JWT provider and then redirected back to your application.

However, there are cases where you may want to use Web3Auth without redirecting the user to a new page. This guide will cover a way to authenticate
users without redirecting them to a new page.

For this particular guide we are using Firebase as our authentication provider, but <b>you can use any provider you want</b>. This particular user
...
## Using the Web3Auth SDK without redirecting
...
3. Initialization of the Web3Auth
4. Initialization of the Torus
...
// this is a generic function which can parse tokens from any JWT token provider
...
Currently, if MFA is disabled we can generate the private key of the user in the backend using the Torus libraries and then pass it to the frontend.
If the MFA is enabled for the user, the user input will be required to generate the private key and then the user will be able to login.

The following code snippet depicts how to enable MFA for users while using Firebase as the JWT provider.

```js
// getting the current user id token
const auth = getAuth(app);
...
const userInfo =  // NEED TO ADD THIS

```

You cannot deactive MFA for a user once its activated.
