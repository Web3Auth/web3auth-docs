...
<SEO
  title="Using Passkeys with Web3Auth SFA SDK"
...
To illustrate how to use the Passkeys SFA Plugin with Web3Auth SFA Web SDK, here is a complete
example.
...
## Caveats of Using Passkeys

- **Device and Browser Limitations:** Passkeys aren't supported on every device. Implementations may
  vary between native apps and Webview environments. Ensure to provide fallbacks for unsupported
  devices and browsers.

- **Domain Restrictions:** Passkeys are currently restricted to the domain they were set up on.
  While this enhances security, it also limits their usability across multiple domains.

- **Passkeys Storage Risks:** Passkeys are often linked to Apple or Google accounts and are synced
  across devices. If these accounts are compromised, users could lose access to their
  passkey-secured accounts.

- **Providing Fallback Options:** It's crucial to provide options like email OTP, SMS OTP, or social
  logins as backups. These are necessary for users on devices or browsers that don't support
  passkeys, or if the passkey flow is canceled.

## Learn More

For detailed documentation and implementation guides, please refer to the
[Passkeys SFA Plugin Documentation](/docs/sdk/passkeys-sfa/passkeys-sfa.mdx).
