...
image: "/docs/contents/guides/banners/firebase.svg"
description: Learn how to use Web3Auth Plug and Play Core SDK with Firebase
type: guide
...
<SEO
  title="Using Firebase with Web3Auth"
  description="Learn how to use Web3Auth Plug and Play Core SDK with Firebase"
  image="https://web3auth.io/docs/contents/guides/banners/firebase.svg"
...
**Live Demo:** https://web3auth-core-firebase-demo.vercel.app
...
![Web3Auth Core - Firebase (Google) Login Flow](/images/implicit-flow.svg)
...
- Next screen shows ways to add **Firebase** to your application. We will be using the confi from here later in the guide. Please keep a note of the
...
- Create a **Verifier** from the **Custom Auth** Section of the [Web3Auth Developer Dashboard](https://dashboard.web3auth.io/) with following
  ...
  ![Custom Authentication - Create AWS Cognito Verifier](/contents/guides/firebase/firebase-custom-verifier.png)
...
console.log("idToken", idToken); // this idToken will be passed to web3auth
```
...
application domain so that you can be redirected after login from the Web3Auth Plug and Play Core SDK.

### Authenticate External Wallet Users
...
If you wish to onboard users via **External Wallets** (such as Metamask, Torus Wallet etc), you can use `authenticateUser()` of Web3Auth SDK. This
will return a `idToken` containing the following details:

#### idToken(Payload) Received for External Wallets
...
```json
{
  ...
  "wallets": [
    {
      "address": "0x032ce04186f175317a3948583a37590f2199f81f",
      "type": "ethereum"
    }
  ],
  "exp": 1661327739
...
[Web3Auth Firebase Example](https://github.com/Web3Auth/examples/tree/main/web-core-sdk/custom-authentication/firebase-react-core-example). Check it
