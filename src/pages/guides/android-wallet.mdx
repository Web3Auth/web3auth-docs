---
title: Create an EVM Web3 wallet in Android
image: "guides/banners/android-wallet.png"
description: Empower your Android app with a EVM Web3 wallet using the Web3Auth PnP SDK.
type: guide
tags: [mobile, android, ethereum]
date: May 27, 2024
...
import WalletPreview from "@site/static/guides/android-wallet-preview.png";

<SEO
  title="Build your own EVM Web3 wallet in Android"
  description="Empower your Android app with a EVM Web3 wallet using the Web3Auth PnP SDK."
  image="https://web3auth.io/docs/guides/banners/android-wallet.png"
  slug="/guides/android-wallet"
/>

In this guide, we'll talk about how we can use Web3Auth to build your EVM Web3 wallet in Android.
The wallet will only support the Ethereum ecosystem, but functionality can be extended with any
...
To install the web3auth-android-sdk SDK,in your module-level `build.gradle` or `settings.gradle`
...
class Web3AuthHelperImpl(
    ...
    // Returns the EVM compatible private key.
...
a new class, `EthereumUseCase` interface, which will used as a base class for `EthereumUseCaseImpl`.
...
For generating the wrappers, we'll use the
[web3j command line tools](https://docs.web3j.io/4.11.0/command_line_tools/).
...
Once, we have installed the cli, the next step is to create `Token.sol` file, which has the smart
...
Once, we have the solc package installed, we'll compile the smart contract. The bin and abi options
will generate the abi and bin files. Feel free to choose the output directory of your choice.

```bash
solc Token.sol --bin --abi --optimize -o <output-dir>/
```

Once, we have compiled the smart contract, the next step is to use the web3j cli to generat the
wrpapers.
...
Once we have the created `EthereumUseCaseImpl`, next is to initialize the `EthereumUseCaseImpl`
...
class MainViewModel(private val web3AuthHelper: Web3AuthHelper) : ViewModel() {
    ...
    // EthereumUseCaseImpl to interact with the selected EVM ChainConfig.
    ...
    // User's EVM compatible private key.
    private fun privateKey(): String {
        return web3AuthHelper.getPrivateKey()
    }
...
Once, we have our view model ready, we create a new `HomeScreen` to show user details as email
...
Voila, you have build a EVM Web3 wallet. This guide only gives you an overview of how to create your
wallet with EVM ecosystem support. The general idea of the guide can be used for any of the
blockchain ecosystem.
