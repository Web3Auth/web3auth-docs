...
- `@tkey/web-storage`: This module is used to store the Device Share and used in the 2/2 flow. If you're building for Web, We recommed you to start
  with this module. i.e `2/2`
- `@tkey/security-questions`: This module is used to create a `Share C: Backup Share` allowing `2/3` flows.
...
Generate a new share(`ShareC: Backup Share`) using the security questions module enabling `2/3` flow.
...
So if you have completed this far, it means that you have successfully authenticated your user and resconstructed the key. Now, you can directly use
the private key to make the RPC calls.

Web3Auth is chain agnostic, you can get the private key in the user scope and make RPC calls to any blockchain.
