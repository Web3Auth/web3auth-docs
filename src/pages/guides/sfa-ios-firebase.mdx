---
title: Using Firebase with Web3Auth iOS SFA SDK
image: "guides/banners/sfa-ios-firebase.png"
description:
  Web3Auth Single Factor Authentication (SFA) iOS SDK with Firebase enables secure blockchain
...
<TabItem value="pods">
Add `SingleFactorAuth` as a dependency to your `Podfile`.

```sh
pod 'SingleFactorAuth', '~> 6.0.0'
```
...
[Learn more about SingleFactorAuth Initialization](/docs/sdk/core-kit/sfa-ios/initialize).
...
authenticated, the result would be a non-nullable `TorusSFAKey`. You can navigate to different views
based on the result.
...
return the `TorusSFAKey`.

Learn more about [SingleFactorAuth LoginParams](/docs/sdk/core-kit/sfa-ios/usage#loginparams). To
more about Firebase login methods, please
...
After successfully installing package, it's time to create an extension for `TorusSFAKey` which
conforms to `EthereumSingleKeyStorageProtocol`.
...
[documentation for iOS SFA SDK](/docs/sdk/core-kit/sfa-ios/). You can find the code used for the
guide on our
