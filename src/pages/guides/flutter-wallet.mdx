...
Head to [Web3Auth's documentation](/docs/dashboard-setup/dashboard-setup) for more on setting up
Web3Auth Dashboard.
...
[Learn more about Web3Auth initiliation](/docs/sdk/pnp/flutter/initialize).
...
Once we have successfully authenticated user, the next step would be to fetch the user details,
...
package. Similary for solana, we'll use the "solana" package. To install the packages, you have two
options. You can either manually add the packages in the `pubspec.yaml` file, or you can use the
...
If you want to learn, how you can intergrate different blockchain with Web3Auth, you can checkout
...
`signMessage`. The `readContract` and `writeContract` specific to Ethereum ecosystem.
...
After having our blockchain proivders are place, the next step on the list to define the supported
...
If you are interested in learning more about Web3Auth, please checkout our documentation for
Flutter. You can find the code used for the guide on our examples repo.
