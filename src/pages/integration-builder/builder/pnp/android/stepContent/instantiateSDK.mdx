---
title: Create and initialize the Web3Auth instance
---

Let's now create an instance of the `Web3Auth`.

We need `clientId` and `network` to initialize the Web3Auth class. You can get your `clientId` from registering (above), whereas `network` signifies
the type of network(`SAPPHIRE_MAINNET`, `MAINNET`, `CYAN`, `AQUA`, `TESTNET` or `SAPPHIRE_DEVNET`) you want to initialize web3auth with.

Now, let's create an instance of the `Web3Auth` class. It can be initialized on-demand when the user interactions are input (like button click) or
when the screen is initialized.

If you want to use Whitelabel or Custom Authentication, you will also have to specify it in the dynamic parameter constructor as well.

:::info sdk reference

Read more about [Android SDK](/sdk/pnp/android).

:::
