...
- `web3AuthNetwork` signifies the network on which the deployed Web3Auth nodes are running. For testing purposes, we have a `testnet` network. For
  production usage, you can use the `cyan`, `mainnet`, `celeste` & `aqua` networks.
...
- For customising the UI for your modal, you can pass on the `uiConfig` parameter to the `Web3Auth` constructor. Know more about
  [Whitelabeling Web3Auth](/pnp/features/whitelabeling)

:::tip SDK Reference

- [Instantiating Web3Auth Modal](/sdk/web/modal/initialize#instantiating-web3auth).
- [Instantiating Web3Auth Core](/sdk/web/no-modal/initialize#instantiating-web3authcore).
