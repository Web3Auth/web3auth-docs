  ...
  build(values: Record<string, string>, files: Record<string, string>) {
    ...
    switch (lang) {
      ...
      case "VUE":
        ...
        // template filenames that your integration page will use
        filenames.push("web3auth/vue/Home.vue"); // Show code files in browsers

        switch (chain) {
          case "Ethereum":
            newFiles["web3auth/vue/Home.vue"] = replaceFileVariable(
              newFiles["web3auth/vue/Home.vue"],
              "const web3AuthCtorParams = {};",
              `        const web3AuthCtorParams = {
              chainConfig: { chainNamespace: "eip155" },
              clientId: "YOUR_CLIENT_ID_HERE", // get your clientId from https://dashboard.web3auth.io
        ...
        steps.push(
          {
            ...STEPS.framework.HTML.installationWeb,
            pointer: { filename: "web3auth/vue/Home.vue", range: "42" },
          },
          {
            ...STEPS.framework.HTML.registerApp,
            pointer: { filename: "web3auth/vue/Home.vue", range: "53" },
          },
          {
            ...STEPS.framework.HTML.instantiate,
            pointer: { filename: "web3auth/vue/Home.vue", range: "44-54" },
          },
          {
            ...STEPS.framework.HTML.subscribe,
            pointer: { filename: "web3auth/vue/Home.vue", range: "72-91" },
          },
          {
            ...STEPS.framework.HTML.initialize,
            pointer: { filename: "web3auth/vue/Home.vue", range: "58" },
          },
          {
            ...STEPS.framework.HTML.triggeringLogin,
            pointer: { filename: "web3auth/vue/Home.vue", range: "96-102" },
          }
          // {
          //   ...STEPS.logout,
    ...
    switch (chain) {
      case "Ethereum":
        ...
        steps.push({
          ...
          pointer: { filename: "eth/ethereum.ts", range: "10-18" },
        });
        break;
      case "Solana":
