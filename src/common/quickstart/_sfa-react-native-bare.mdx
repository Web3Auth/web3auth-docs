...
Install the Web3Auth's Single Factor Auth package in your react-native project. When using our SDK with a bare workflow React Native app, you have to install a
`WebBrowser` implementation made by us and a `Storage` implementation provided by react-native.

```bash npm2yarn
npm install --save @web3auth/single-factor-auth-react-native @web3auth/ethereum-provider react-native-encrypted-storage
```
