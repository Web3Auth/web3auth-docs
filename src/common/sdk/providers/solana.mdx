...
If you are using `chainNamespace: CHAIN_NAMESPACES.SOLANA` while initializing `Web3Auth` or
`Web3AuthNoModal` with the `OpenloginAdapter`, you need to add the `privateKeyProvider` to the
OpenLogin instance.
