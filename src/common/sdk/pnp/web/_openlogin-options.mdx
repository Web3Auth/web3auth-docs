...
| `network`               | `enum` (`testnet`, `mainnet`, `cyan`, `aqua`, `celeste`) | Network specifies the openlogin iframe url url to be used. <ul><li>`'mainnet'`: https://app.openlogin.com will be used which is the production version.</li><li>`'cyan'`: https://cyan.openlogin.com will be used which is the production cyan version.</li><li>`'testnet'`: https://beta.openlogin.com will be used which is the beta version.</li></ul> | Yes       | N/A           |
...
export declare type OpenLoginOptions = {
   ...
   * Not required if you're passing the clientId in the web3auth core constructor.
   */
  clientId?: string;
  /**
