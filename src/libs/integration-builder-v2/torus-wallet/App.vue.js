    ...
    >
      ...
      <section :style="{ marginTop: '20px' }">
        ...
        <div :style="{ marginTop: '20px' }">
          <select
            ...
            @change="onSelectedVerifierChanged"
          >
      ...
      <section :style="{ marginTop: '20px' }">
        ...
        <section>
          ...
          <div>
            <input
              :style="{ marginReft: '20px' }"
              v-model="messageToEncrypt"
              placeholder="Message to encrypt"
            />
            <button :disabled="!encryptionKey" @click="encryptMessage">
              Encrypt
            </button>
          </div>
          <button :disabled="!messageEncrypted" @click="decryptMessage">
            Decrypt
          </button>
        </section>
      </section>
...
export default {
  ...
  methods: {
    ...
    console(...args) {
      document.querySelector("#console>p").innerHTML = JSON.stringify(
        args || {},
        null,
        2
      );
    ...
    sendEth() {
      window.web3.eth
        .sendTransaction({
          ...
          value: window.web3.utils.toWei("0.01"),
        })
    ...
    async signPersonalMsg() {
      try {
        ...
        const hostnamealAddress = await window.web3.eth.personal.ecRecover(
          hash,
          sig
        );
        if (
          this.publicAddress.toLowerCase() === hostnamealAddress.toLowerCase()
        )
